...
The triple difference estimator essentially takes two DDs, one with the target unit of analysis with a treated and an untreated group. This is compared to another similar group in the pre and post-treatment period. So effectively there are two treatments. One where an actual treatment on the desired group is tested, and a placebo comparison group, on which the same intervention is also applied.
...
According to the last line, the treatment effect should have an impact of 2 units on Y in the post group. We can check this by plotting the data:
...
Let's start with a simple case where we have one control group and two treatment groups. The two treatment groups receive treatment at the same time but with different treatment intensities:
...
Here we can see that the post treatment has an average effect of 2 on id=2 and 4 on id=3. This implies that the ATT equals $$ \beta^{TWFE} $$=3, which we can also check by recovering the coefficients:
...
From the earlier example, we know that the ATT equals $$ \beta^{TWFE} $$=3, but from the graphs we cannot see this so clearly. This is because we need to get rid of panel and id time trends. While we can also do this partialling out by hand (but we won't), we can use our regression specification:
...
Here, I would like to add that parallel trend assumptions are controlled for in the above regression specification. If these are not accounted for, then we basically end up with the wrong ATTs. We can see the D coefficients in the following regressions:
...
Unlike the previous examples, where we could derive the ATT just by looking at the graph, it is not so trivial here. Even though there are no time and panel fixed effects, differentials in treatment time do make changes over panel and time relevant. Without going into the maths, to recover the actual ATT, we need to average out time and panel effects for treated and non-treated observations.
...
It is these combinations that are unraveled in the section on [Bacon decomposition](https://asjadnaqvi.github.io/DiD/docs/code_stata/06_02_bacon/), which is why it is important to understand the decomposition carefully.
