...
. Rather than writing one large table, it is easier to present results for C = 0 (main group) and C = 1 (comparison group). The difference between those two DDs is exactly
$$
\beta_7
$$
...
You can also estimate the same design as OLS with explicit fixed effects:
...
$$
 ...
 increases Y by about 1 unit per period, as expected. The intercept is 1.5 (the average implied baseline at
$$
t=0
$$

 across both units), and
$$
\beta^{TWFE} = 3
$$
...
As before, the ATT is

$$
\beta^{TWFE}=3
$$

, but it is no longer obvious from the raw plot because unit and time patterns are mixed in. Rather than partialling out by hand, we estimate:
...
Now we move to staggered adoption where groups are treated at different times. We again use a generated dataset and then recover

$$
\beta^{TWFE}
$$

 through regression with fixed effects.
...
In this figure, id=2 is treated from

$$
t=5
$$

 onward, while id=3 is treated from
$$
t=8
$$

 onward. So what is ATT in this case?
...
The last specification gives the TWFE estimate,

$$
\beta^{TWFE} \approx 2.91
$$

. This is not a simple treated-cell mean; it is an FE-weighted average after removing unit and time effects.
...
Both commands return the same estimate,
...
Important: with staggered treatment, use full time fixed effects (`i.t`), not a single linear trend (`t`). The dofile flags this explicitly:
...
Why is this happening? Because with staggered timing and heterogeneous effects, "pre/post" and "treated/untreated" are not single comparisons anymore. In

$$
5 \leq t < 8
$$

, only id=2 contributes new treatment variation. For
$$
t \geq 8
$$

, id=3 begins treatment while id=2 is already treated.
