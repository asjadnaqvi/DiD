...
which event_plot   // ver 01 June 2021

ssc install reghdfe, replace
which reghdfe     // 6.12.3 (08 Aug 23)

ssc install ftools, replace  // required by reghdfe
which ftools   // 2.49.1 (08 Aug 23)
...
which did_imputation  // 23 Apr 2023

ssc install avar, replace
which avar    // 1.0.07 28 Jul 2015

ssc install eventstudyinteract, replace
which eventstudyinteract // v0.1 24 Jan 2022

ssc install did_multiplegt, replace
which did_multiplegt  // no version

ssc install stackedev, replace
which stackedev  // no version
...
*jwdid Y, ivar(id) time(t) gvar(gvar) 
*estat event, estore(jwdid) 
